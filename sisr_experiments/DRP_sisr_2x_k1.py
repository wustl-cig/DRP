...
def main():
    ...
    model_name = 'DRP'
    testset_name = 'set5'  # test set,  'set3c'| 'set5' | 'cbsd68'| 'McMaster'
    sf = 2  # set scale factor, default: 3
    border = sf
    iter_num = 30 # set number of iterations, default: 24 for SISR
    ...
    for idx, img in enumerate(L_paths):
        ...
        if save_E:
            util.imsave(img_E, os.path.join(E_path, img_name + '_x' + str(sf) + '_k:1.6' + '_' + 'Recon' + '.png'))
        ...
        if save_L:
            util.imsave(img_L,
                        os.path.join(E_path, img_name + '_x' + str(sf) + '_k:std = 1.6' + '_LR.png'))

        psnr = util.calculate_psnr(img_E, img_H, border=border)

        test_results['psnr'].append(psnr)
        logger.info(
                    '{:->4d}--> {:>10s} -- sf:{:>1d} --k:std = 1.6 PSNR: {:.2f}dB'.format(idx + 1, img_name + ext, sf, psnr))
    # ---------------------------------------
